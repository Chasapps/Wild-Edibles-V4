...
/* Wild Fruit Mapper — Pro (Categories + Catalog + Import) */

const LS_KEY = 'wild_fruit_trees_v1';
const MASTER_LS_KEY = 'wild_fruit_master_v1';

// Legacy emoji map (fallback)
const FRUIT_EMOJI = {
...
// Lite defaults so the app works before a catalog is loaded
...
// Filter panel type list (legacy list)
...
// Run once and on change (Safari-friendly: change + input)
document.addEventListener('DOMContentLoaded', refillFruitOptions);
categorySelect?.addEventListener('change', refillFruitOptions);
categorySelect?.addEventListener('input', refillFruitOptions);
...
function openEdit(id){
  ...
  refillFruitOptions();
  const names = getNamesForCategory(categorySelect.value);
  fruitSelect.value = names.includes(t.type) ? t.type : 'Other';
...
function importGeoJSONFile(file){
  ...
  reader.onload = () => {
    try{
      ...
      // Accept catalog JSON via Import too
      if (fc && fc.native_fruits && fc.common_fruits && fc.edible_plants){
        MASTER = fc; localStorage.setItem(MASTER_LS_KEY, JSON.stringify(MASTER));
        refillFruitOptions(); toast('Catalog loaded ✅'); return;
      }
      ...
      saveTrees(); renderAll(); toast(`Imported ${imported.length} point(s)`);
...
// Ensure menu is ready even if dialog opens immediately
